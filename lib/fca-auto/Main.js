...
function buildAPI(globalOptions, html, jar) {
    ...
    if (!userID && global.Fca && global.Fca.Data && global.Fca.Data.AppState) {
        const savedCuser = global.Fca.Data.AppState.find(c => c.key === "c_user" || c.key === "i_user");
        if (savedCuser) userID = savedCuser.value;
    }
                            ...
                            if (html.indexOf("checkpoint/?next") > -1) {
                                ...
                                switch (global.Fca.Require.FastConfig.Login2Fa) {
                                    case true: {
                                        ...
                                        const _codeValue = global.Fca.Require.FastConfig.AuthString.includes('|') == false
                                            ? Promise.resolve(Otp_code(global.Fca.Require.FastConfig.AuthString.includes(" ") ? global.Fca.Require.FastConfig.AuthString.replace(RegExp(" ", 'g'), "") : global.Fca.Require.FastConfig.AuthString))
                                            : question(Language.EnterSecurityCode);
                                        return _codeValue.then(Code => { try {
                                                        ...
                                                        if (typeof N_Code == "string") {
                                                            utils
                                                                ...
                                                                .then(function(res) {
                                                                    ...
                                                                    if (error) {
                                                                        logger.Warning(Language.InvaildTwoAuthCode, function() { question(Language.EnterSecurityCode).then(c => approvals(c)); }); //bruh loop
                                                                    };
                                                                })
                                                                .then(function() {
        ...
        if (appState) {
            ...
            try {
                ...
                appState.map(function(c) {
                    var str = c.key + "=" + c.value;
                    if (c.expires && c.expires !== "Infinity") str += "; expires=" + new Date(c.expires).toUTCString();
                    str += "; domain=" + c.domain + "; path=" + (c.path || "/");
                    try {
                        var cookieDomain = c.domain.replace(/^\./, "");
                        var cookieUrl = "https://www." + cookieDomain;
                        jar.setCookie(str, cookieUrl);
                    } catch (cookieErr) {}
                });
                ...
                logger.Warning("AppState processing error: " + (e && e.message ? e.message : String(e)));
