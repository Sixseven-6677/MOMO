...
function getDisabled() {
  try {
    if (fsmod.existsSync(disabledPath)) return JSON.parse(fsmod.readFileSync(disabledPath, "utf8"));
  } catch (e) {}
  return [];
}

function saveDisabled(list) {
  try { fsmod.writeFileSync(disabledPath, JSON.stringify(list, null, 2), "utf8"); } catch(e) {}
}
...
module.exports.config = {
  ...
  usages: "قروبات | قروبات تعطيل [رقم] | قروبات تفعيل [رقم] | قروبات اضافة [رقم] | قروبات مغادرة [رقم]",
  cooldowns: 0
};
...
module.exports.run = async function({ api, event, args }) {
  ...
  if (args[0] === "مغادرة") {
    ...
    if (isNaN(num) || num < 1 || num > fallback.length)
      return api.sendMessage("❌ رقم غير صحيح، اكتب قروبات أولاً ثم قروبات مغادرة [رقم]", threadID, messageID);
    ...
    try {
      ...
      return api.sendMessage("✅ تم مغادرة القروب:\n" + name, threadID, messageID);
    } catch (e) {
      return api.sendMessage("❌ فشلت المغادرة من: " + name + "\nقد لا أكون أدمن في ذلك القروب", threadID, messageID);
    }
  }
  ...
  if (args[0] === "تعطيل" || args[0] === "تفعيل") {
    const num = parseInt(args[1]);
    if (isNaN(num) || num < 1 || num > fallback.length)
      return api.sendMessage("❌ رقم غير صحيح، اكتب قروبات أولاً لتحديث القائمة", threadID, messageID);
    ...
    if (args[0] === "تعطيل") {
      ...
      return api.sendMessage("🔴 تم تعطيل البوت في:\n" + name + "\nللتفعيل: قروبات تفعيل " + num, threadID, messageID);
    }
    ...
    if (args[0] === "تفعيل") {
      ...
      return api.sendMessage("🟢 تم تفعيل البوت في:\n" + name, threadID, messageID);
    }
  }
  ...
  if (args[0] === "اضافة") {
    ...
    try {
      ...
      return api.sendMessage("❌ فشل الإضافة إلى: " + name, threadID, messageID);
    }
  }
  ...
  return api.sendMessage(
    ...
    "قروبات مغادرة [رقم] ← البوت يغادر القروب",
    threadID, messageID
  );
};
