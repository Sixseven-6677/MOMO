...
module.exports.run = async function({ api, event }) {
  ...
  const expNeeded = EXP_NEEDED(p.level);
  ...
  const pathLabel = p.path ? `${p.path} (درجة ${(p.pathTiers||[]).length}/4)` : "لم يُختر بعد";
  ...
  const msg =
    ...
    `╚══════════════════════╝\n` +
    `━━━━━━━━━━━━━━━━━\n` +
    `${rankIcon} الرتبة: ${p.rank} | المستوى: ${p.level}\n` +
    `📊 XP: [${bar}] ${p.exp}/${expNeeded}\n` +
    `━━━━━━━━━━━━━━━━━\n` +
    `❤️  HP:     ${p.maxHP}\n` +
    `⚔️  هجوم:   ${p.atk}\n` +
    `🛡  دفاع:   ${p.def}\n` +
    ...
    `🔮 تعاويذ: ${spells}\n` +
    `━━━━━━━━━━━━━━━━━\n` +
    `🌿 مسار التطوير: ${pathLabel}\n` +
    `🛡 المهارة السلبية: ${passive}\n` +
    `━━━━━━━━━━━━━━━━━\n` +
    `🪙 ذهب: ${p.coins || 0}\n` +
    `⭐ هيبة: ${p.reputation || 0}\n` +
    `🏆 انتصارات: ${wins} | خسائر: ${losses} | نسبة الفوز: ${winRate}%\n` +
    `━━━━━━━━━━━━━━━━━\n` +
